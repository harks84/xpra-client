...
public class ConnectorUtil {
	...
	public static XpraConnector getConnector(XpraClient client, String connString) {
		...
		case TCP_PROTOCOL:
			...
			return new TcpXpraConnector(client, user, host, port);
		case WS_PROTOCOL:
			if (port == null) {
				port = 80;
			}
			return new WsXpraConnector(client, user, host, port, false);
		case WSS_PROTOCOL:
			if (port == null) {
				port = 443;
			}
			return new WsXpraConnector(client, user, host, port, true);
		case SSH_PROTOCOL:
			if (port == null) {
				port = 22;
			}
			return new SshXpraConnector(client, user, host, port);
